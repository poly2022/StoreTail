            ...
            <table id="example1" class="table table-bordered table-striped">
                ...
                <?php
                    $query = "SELECT subscriptions.*, users.user_name FROM subscriptions
                            ...
                            WHERE subscriptions.plans_id = 1";
                    ...
                    if (mysqli_num_rows($query_run) > 0) {
                        foreach ($query_run as $row) {
                            ...
                            <tr>
                                ...
                                <td><?php echo ($row['plans_id'] == 1) ? 'Premium' : ''; ?></td>
                                <td>
                                    <form action="new_premiumcode.php" method="post">
                                        <input type="hidden" name="users_id" value="<?php echo $row['users_id']; ?>">
                                        <button type="submit" name="revokebtn" class="btn btn-danger btn-sm deletebtn">Revoke</button>
                                    </form>
                                </td>                                
